...
public class FavoriteController {
  ...
  public BaseResponse<FavoriteStatusResponse> status(
      ...
      @Parameter(description = "찜 대상 타입 (예: INGREDIENT, RECIPE)", required = true)
      @RequestParam FavoriteType type,
      @Parameter(description = "찜 대상 ID", required = true)
  ...
  public BaseResponse<FavoriteStatusResponse> like(
    ...
    return BaseResponse.success("찜 등록 성공",
        favoriteService.like(me.getId(), req.type(), req.targetId()));
  }
  ...
  public BaseResponse<Page<IngredientCardDto>> favoriteIngredients(
      @AuthenticationPrincipal CustomUserDetails me,
      @Parameter(description = "페이지네이션 정보 (기본: size=20, sort=likedAt DESC)")
      @PageableDefault(size = 20, sort = "likedAt", direction = Sort.Direction.DESC)
      Pageable pageable
  ) {
    return BaseResponse.success(
        "찜한 식재료 목록 조회 성공", favoriteRepository.findIngredientCards(me.getId(), pageable));
  }
  ...
  public BaseResponse<Page<RecipeCardDto>> favoriteRecipes(
    ...
    return BaseResponse.success(
        "찜한 레시피 목록 조회 성공", favoriteRepository.findRecipeCards(me.getId(), pageable));
  }
}