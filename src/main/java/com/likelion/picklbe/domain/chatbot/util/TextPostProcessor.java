...
public class TextPostProcessor {
  ...
  // ",운동" → ", 운동" 같은 콤마 뒤 공백 보정
  private static final Pattern COMMA_TIGHT = Pattern.compile(",(?=\\S)");

  // "운동30분" / ",운동30" / "운동 30 분" 등을 ", 운동 30분"으로 정규화
  private static final Pattern EXERCISE_TIGHT = Pattern.compile("(?i)(,?)\\s*운동\\s*(\\d+)\\s*분?");
  ...
  private static final Pattern SPACE_BEFORE_DIGIT = Pattern.compile("(?<=[가-힣A-Za-z)])(?=\\d)");
