...
public class ChatDtos {
  ...
  public record ChatRequest(
      ...
      @Schema(description = "사용자 메시지", example = "대한민국 수도는?")
      String message
  ) {}
  ...
  public record HistoryRequest(
      ...
      @Schema(description = "대화 ID", example = "42")
      Long conversationId,

      @Schema(description = "히스토리 턴 목록")
      List<HistoryTurn> messages
  ) {}

  // 백 → 프론트
  @Schema(name = "ChatResponse", description = "챗봇 응답")
  public record ChatResponse(
      @Schema(description = "대화 ID(새로 생성되었거나 기존)", example = "42")
      Long conversationId,

      @Schema(description = "어시스턴트 응답", example = "대한민국의 수도는 서울입니다.")
      String reply
  ) {}
}